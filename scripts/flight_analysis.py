...
from pyspark.sql.functions import (
    col, explode, min, max, month, when, count, avg, round, desc, from_unixtime,
...
def analyze_bookings(bookings_df, airports_df):
    ...
    enriched_df = enriched_df.withColumn(
        "minutes_to_add", col("timezone_offset") * 60
    ).withColumn(
        "local_departure",
        from_unixtime(
    ...
    ).withColumn("day_of_week", date_format(col("local_departure"), "EEEE"))

    # Group and aggregate the data with age statistics
    popularity = (
        enriched_df.groupBy(col("airports.country"), "season", "day_of_week")
        .agg(
...
if __name__ == "__main__":
    ...
    if args.csv:
        ...
        logging.info("Results saved as CSV at: %s", csv_path)

    if args.parquet:
        parquet_path = f"{OUTPUT_PATH}/output.parquet"
        results.write.mode("overwrite").parquet(parquet_path)
        logging.info("Results saved as Parquet at: %s", parquet_path)
