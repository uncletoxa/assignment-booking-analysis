...
def load_airport_data(spark, csv_path):
    """
    Load airport reference data including timezone name, dropping invalid timezones
    """
    return spark.read.option("header", "false") \
        ...
        .select(
            ...
            col("_c4").alias("iata"),
            col("_c11").alias("timezone")
        ) \
        .filter(col("iata").isNotNull()) \
        .filter(col("timezone") != "\\N")


def load_booking_data(spark, json_path, start_date, end_date):
    """
    ...
    df = df.select(
        col("flights.flight.marketingAirline").alias("airline"),
    ...
    # Filter for confirmed bookings and date range
    df = df.filter(
        col("status") == "CONFIRMED"
    ).filter(
        (col("departure_date") >= start_date) &
        (col("departure_date") <= end_date)
    )
...
def analyze_bookings(bookings_df, airports_df):
    ...
    """
    # Join with airports to get timezone information
    ...
    # Convert UTC departure time to local time
    enriched_df = enriched_df.withColumn(
        "local_departure",
        from_utc_timestamp(col("departure_date"), col("timezone"))
    )
